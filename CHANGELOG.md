...
Загрузочный экран при старте и накопленные улучшения с v0.10.1.

### Added

- **Splash-экран** при запуске GUI: брендинг, этапы загрузки, прогресс, ожидание prewarm Playwright (отключение: `SCENARIA_SKIP_SPLASH=1`).

### Changed

- Поэтапный старт приложения: тяжёлые импорты и инициализация окна — за splash (`app/qt/startup.py`).
- Внутренний рефакторинг (спринты 15–20): split `player` и `gherkin_ru`, mypy на MVC, стабильный CI без skipped-тестов.
...
[0.10.1]: https://github.com/bafgion/scenaria/compare/v0.8.1...v0.10.1
