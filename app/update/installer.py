...
def prepare_update_script(
    ...
    *,
    ...
    lines = [
        ...
        "goto wait_exit",
        ":force_kill",
        'echo [%date% %time%] Force kill PID %PID% >>"%LOG%"',
        "taskkill /PID %PID% /T /F >>\"%LOG%\" 2>&1",
        f'taskkill /IM {exe_name} /T /F >>"%LOG%" 2>&1',
        "ping -n 3 127.0.0.1 >nul",
        ":do_copy",
        'echo [%date% %time%] Copying update files >>"%LOG%"',
        (
            'robocopy "%STAGING%" "%TARGET%" /E /XD data browsers /R:5 /W:2 '
            '/NFL /NDL /NJH /NJS /NC /NS /NP >>"%LOG%" 2>&1'
        ),
        "set RC=%ERRORLEVEL%",
        'echo [%date% %time%] robocopy exit %RC% >>"%LOG%"',
        "if %RC% GEQ 8 (",
        '  echo Update failed, robocopy code %RC% >>"%LOG%"',
        "  exit /b %RC%",
        ")",
